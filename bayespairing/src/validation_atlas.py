...
graphs = pickle.load(open("../models/bp2june2_one_of_each_graph.cPickle", "rb"))
pdbs = pickle.load(open("../models/bp2june2_PDB_names.cPickle", "rb"))
mod_sequences,test_sequences, all_indexes = pickle.load(open("../models/bp2june2_sequences.pickle", "rb"))
...
def run_validation(module_to_test,full_seq,LEFT_OUT,left_out_seq, leave_out_seq, aiming_for, indexes):
    ...
    if i in siblings:
        ...
        sibs = []
    maxs = parse_sequences.run_BP(full_seq, "", [i]+sibs, DATASET_NAME, left_out=LEFT_OUT,leave_out_sequence=leave_out_seq,left_out_sequence=left_out_seq,indexes=indexes,samplesize=20000)
    ...
    if i in siblings:
        print("SEARCHING FOR GROUP OF MODULE",module_to_test,"SIBLINGS",siblings[i],"FOUND",maxs.keys())
    else:
        print("SEARCHING FOR GROUP OF MODULE",module_to_test,"NO SIBLINGS",i,"FOUND",maxs.keys())
...
def cross_validation(modules_to_test,it):
    ...
    for i in modules_to_test:
        if i in TO_SKIP: #we have done a sibling
            continue
        ...
        cv_fn = "../output/juneTESTfold_"+it + str(i)
   ...
   #     #print(crossval_results)
    pickle.dump(crossval_results, open(str("june1fold_all_"+it+".pickle"), "wb"))
    print('FINAL RESULTS')
    for i in crossval_results:
...
if __name__ == "__main__":
    it = "JUNE24_NOSS_"
    modules_to_test = list(range(100,len(graphs)))
    #modules_to_test = list(range(0,100))
    #modules_to_test = [159]
