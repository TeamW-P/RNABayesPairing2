...
def cross_validation(modules_to_test,it="A"):
    done_seqs = []
    crossval_results = {}
    CV_SEQUENCES = []
    for i in modules_to_test:
        ...
        for ind in indexes:
            ...
            if pdb_len in range(10, 200) and "T" not in seq and "-" not in seq and "N" not in seq:
                ...
                #    continue
                #3done_seqs.append(seq)
                seq,aiming_for=correct_aiming_for(seq,aiming_for)
                shuffled_seq = shuffle_seq(seq.upper())
                scores = run_validation(i,shuffled_seq,ind, leave_out_seq=False,left_out_seq=MODULE_SEQUENCES[ind],aiming_for=aiming_for,indexes=indexes)
                ...
                if last - first < 200:
                    ...
                    shuffled_seq = shuffle_seq(new_seq)
                    scores = run_validation(i,shuffled_seq,ind,leave_out_seq=False,left_out_seq=MODULE_SEQUENCES[ind], aiming_for=aiming_for,indexes=indexes)
                    #except:
        ...
        cv_fn = "../output/shuffled_"+it + str(i)
   ...
   #     #print(crossval_results)
    pickle.dump(crossval_results, open("shuffled_"+it+".pickle", "wb"))
    print('FINAL RESULTS')
    for i in crossval_results:
