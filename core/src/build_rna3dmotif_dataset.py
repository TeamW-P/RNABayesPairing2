...
modules = pickle.load(open("../src/all_modules_all_examples.cPickle","rb"))
...
for ind, mod in enumerate(graphs):
    ...
    if len(mod_graphs)>2:
        try:
            ...
            print("TERRIBLE")
            continue
pickle.dump(full_aln, open("rfam_rna3dmotif_aligned_modulegraphs.cPickle", 'wb'))
pickle.dump(all_graphs, open("rfam_rna3dmotif_one_of_each_graph.cPickle", 'wb'))
pickle.dump(PDBs, open("rfam_rna3dmotif_PDB_names.cPickle", 'wb'))
pickle.dump(motif_pos, open("rfam_rna3dmotif_PDB_positions.cPickle", 'wb'))
