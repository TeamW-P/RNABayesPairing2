...
class BayesianModelSampling(Inference):
    ...
    def __init__(self, model):
        ...
        print("initialized BN with topological order",type(model),list(nx.topological_sort(model)))
    ...
    def likelihood_weighted_sample(self, evidence=None, size=1, return_type="dataframe"):
        ...
        """
        types = [(str(var_name), np.int8) for var_name in self.topological_order]
        types.append(('_weight', 'float'))
        sampled = np.zeros(size, dtype=types).view(np.recarray)
        sampled['_weight'] = np.ones(size)
        evidence_dict = {var: st for var, st in evidence}
        print("Sampling",self.topological_order)
        for node in self.topological_order:
            cpd = self.model.get_cpds(node)
            #print("this_cpd",cpd)
            states = range(self.cardinality[node])
            ...
            if evidence:
                evidence_values = np.vstack([sampled[str(i)] for i in evidence])
                ...
                if node in evidence_dict:
                    sampled[str(node)] = evidence_dict[node]
                    for i in range(size):
                        sampled['_weight'][i] *= weights[i][evidence_dict[node]]
                else:
                    sampled[str(node)] = sample_discrete(states, weights)
            else:
                if node in evidence_dict:
                    ...
                    sampled[str(node)] = sample_discrete(states, cpd.values, size)
        return _return_samples(return_type, sampled)
...
class GibbsSampling(MarkovChain):
    ...
    >>> from src.pgmpy.inference import GibbsSampling
    ...
    def sample(self, start_state=None, size=1, return_type="dataframe"):
        ...
        Examples:
        ---------
        >>> from src.pgmpy.factors import DiscreteFactor
        >>> from src.pgmpy.inference import GibbsSampling
