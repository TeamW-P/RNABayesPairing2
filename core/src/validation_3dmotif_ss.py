...
mod_sequences,test_sequences, all_indexes = pickle.load(open("../models/RFAM3dmotif2_sequences.pickle", "rb"))
...
def get_widest_graph(g,aiming_fors):
    new_nodes = {}
    selected_ind = -1
    max_size = -1
    for ind,aiming_for in enumerate(aiming_fors):
        size = aiming_for[-1] - aiming_for[0]
        if size>max_size:
            max_size=size
            selected_ind = ind
...
def run_validation(module_to_test,full_seq,LEFT_OUT,left_out_seq, leave_out_seq, aiming_for, indexes, ss=""):
    results = {}
    ...
    maxs = parse_sequences.run_BP(full_seq, ss, [i], DATASET_NAME, left_out=LEFT_OUT,leave_out_sequence=leave_out_seq,left_out_sequence=left_out_seq,indexes=indexes,samplesize=20000)
...
def cross_validation(modules_to_test,it):
    ...
    for i in modules_to_test:
        ...
        #if len(THESE_SEQUENCES)>100:
        #    indexes = get_random_indexes(len(THESE_SEQUENCES))
        #else:
        #    indexes = list(range(len(THESE_SEQUENCES)))
        ...
        for ind in indexes:
            ...
            if pdb_len in range(10, 200) and "T" not in seq and "-" not in seq and "N" not in seq:
                ...
                #print("BEFORE ADJUSTMENT;",aiming_for,len(seq))
                seq,aiming_for=correct_aiming_for(seq,aiming_for)
                #print("AFTER ADJUSTMENT;",aiming_for,len(seq))
                motif_positions=aiming_for
                constraints = testSS.get_constraints_from_BN(graphs[i][ind], motif_positions)
                    #print('CONSTRAINTS', constraints)
                constrained_ss = ["."]*len(seq)
                for pp, cons in enumerate(constraints):
                    constrained_ss[motif_positions[pp]] = constraints[pp]
                constraints = "".join(constrained_ss)
                print("CALLING RNAFOLD WITH CONSTRAINTS", constraints)
                custom_ss = testSS.call_rnafold(seq, cons = constraints, just_ss = True)
                print(custom_ss)
               
                scores = run_validation(i,seq.upper(),ind, leave_out_seq=False,left_out_seq=MODULE_SEQUENCES[ind],aiming_for=aiming_for,indexes=indexes,ss=custom_ss)
                ...
                if last - first < 200:
                    ...
                    #modified to generate the sequences, uncomment everything between =
                    #========================================================
                    #try:
                    #if new_seq in done_seqs:
                    #    print("THIS SEQ WAS TESTED BEFORE")
                    #    continue
                    #done_seqs.append(new_seq)
                    ...
                    #print("AFTER ADJUSTMENT;",aiming_for,len(new_seq))
                    modpos = aiming_for
                    constraints = testSS.get_constraints_from_BN(graphs[i][ind], modpos)
                    ...
                    scores = run_validation(i,new_seq,ind,leave_out_seq=False,left_out_seq=MODULE_SEQUENCES[ind], aiming_for=aiming_for,indexes=indexes,ss=custom_ss)
                    #except:
   ...
   #     #print(crossval_results)
    pickle.dump(crossval_results, open(str("ss"+it+").pickle"), "wb"))
    print('FINAL RESULTS')
    for i in crossval_results:
